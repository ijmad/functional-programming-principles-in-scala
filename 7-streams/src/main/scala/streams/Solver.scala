...
trait Solver extends GameDef {
  ...
  def from(initial: Stream[(Block, List[Move])], explored: Set[Block]): Stream[(Block, List[Move])] = {
    def fromNext(block: Block, tail: Stream[(Block, List[Move])], neighbors: Stream[(Block, List[Move])]): Stream[(Block, List[Move])] =
      neighbors ++ from(tail ++ neighbors, explored + block)

    initial match {
      case Stream.Empty => Stream.Empty
      case (block, moves) #:: tail => fromNext(block, tail, newNeighborsOnly(neighborsWithHistory(block, moves), explored + block))
    }
  }
